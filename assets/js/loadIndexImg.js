document.addEventListener("DOMContentLoaded", function () {
    ...
    (function () {
        ...
        bg2.style.willChange = "transform, opacity";
        content.style.willChange = "transform, opacity";
        bodyico.style.willChange = "transform, opacity";
    
        // Mobile-specific function
        const mobileAnimation = () => {
            const maxTranslateBgX = -5;
            const maxTranslateBgY = -4;
            const maxTranslateBg2X = 1;
            const maxTranslateBg2Y = 4;
            const maxTranslateContentY = -8;
    
            const updateTransforms = (scrollRatio) => {
                const opacity = Math.max(1 - scrollRatio, 0);
                const scale = 1 + scrollRatio * 0.2; // Reduced scale factor for mobile
        ...
        const desktopAnimation = () => {
            const maxTranslateBgX = -8;
            const maxTranslateBgY = -6;
            const maxTranslateBg2X = 2;
            const maxTranslateBg2Y = 8;
            const maxTranslateContentY = -8;
    
            const updateTransforms = (scrollRatio) => {
                const opacity = Math.max(1 - scrollRatio, 0);
                const scale = 1 + scrollRatio * 0.15; // Slower scale factor for desktop for smoother animation
                ...
                content.style.opacity = Math.max(1 - scrollRatio * 1.2, 0).toFixed(2);
                content.style.transform = `translateY(${translateContentY}%)`;
    
                bodyico.style.opacity = Math.max(1 - scrollRatio * 1.2, 0).toFixed(2);
                bodyico.style.transform = `translateY(${translateContentY}%)`;
            };
        ...
        const checkPosition = (updateTransforms) => {
            const scroll = window.scrollY || window.pageYOffset;
    
            if (scroll > height / 2 && !contentIsVisible) {
                showCounterContent();
            }
    
            body.style.display = scroll > height / 2 ? "none" : "";
    
            const scrollRatio = Math.min(scroll / (height / 1.5), 1); // Reduced height threshold for faster animations
        ...
        const onScroll = (updateTransforms) => {
            if (!ticking) {
                requestAnimationFrame(() => {
                    checkPosition(updateTransforms);
                    ticking = false;
                });
                ticking = true;
            }
        ...
        // Select mobile or desktop function
        const updateTransforms = isMobile ? mobileAnimation() : desktopAnimation();
    
        window.addEventListener("scroll", () => onScroll(updateTransforms));
    })();
