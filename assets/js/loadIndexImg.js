document.addEventListener("DOMContentLoaded", function () {
    ...
    (function () {
        ...
        const updateDesktopTransforms = (scrollRatio) => {
        ...
        const updateMobileTransforms = (scrollRatio) => {
            ...
            const opacity = Math.max(1 - scrollRatio, 0);
            const scale = 1 + scrollRatio * 0.2;  // Adjust scale factor for mobile
            ...
            bg.style.opacity = (opacity * 2).toFixed(2);  // Keep the same for bg
            bg.style.transform = `scale(${scale.toFixed(2)}) translate(${translateBgX}%, ${translateBgY}%)`;
            bg2.style.opacity = Math.max(0, 1 - scrollRatio * 2).toFixed(2);
            bg2.style.transform = `scale(${scale.toFixed(2)}) translate(${translateBg2X}%, ${translateBg2Y}%)`;
        ...
        const checkPosition = () => {
            ...
            const scrollRatio = Math.min(scroll / (height / 2), 1);
            if (window.innerWidth < 768) {
                updateMobileTransforms(scrollRatio);
            } else {
                updateDesktopTransforms(scrollRatio);
            }
        };
